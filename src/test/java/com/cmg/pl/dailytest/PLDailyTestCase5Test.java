...
public class PLDailyTestCase5Test {
	...
	public void testPLDailyTestCase5() throws Exception {
		...
		System.out.println("Case #5: Begin to load #5 member BPF-0100027(Pensioner)!");
		try {
			...
			// Finish
			System.out.println("Case #5: Loading #5 member BPF-0100027(Pensioner) PASS!");
			// ----------------------------------------
			// May take a snapshot here
			TakeScreenShot.takeScreenshoot();
		} catch (SeleniumException se) {
			retried++;
			System.out.println("retried case5:" + retried);
			System.out.println(se.getMessage());
			if (retried < 5) {
				testPLDailyTestCase5();
			} else{
				System.out.println("Case #5: Loading #5 member BPF-0100027(Pensioner) FAILURE!");
			}
		}
