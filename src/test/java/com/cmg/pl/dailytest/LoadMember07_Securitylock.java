...
public class LoadMember07_Securitylock {
	...
	public void LoadMember07Securitylock() throws InterruptedException {
		...
		Assert.assertTrue(CheckSchemeBenefitsPage.checkTablePersonalDetailsExisted(driver,Constant.SMALL_WAITING_TIME));
		Reporter.log("Then Check the table personal details will show");
		Assert.assertTrue(CheckSchemeBenefitsPage.checkMemberRefNumberExisted(driver, ParameterMap.getValue("refno")));
		...
		Assert.assertTrue(CheckStateBenefitsPage.checkTablePersonalDetailExisted(driver,Constant.SMALL_WAITING_TIME));
		Reporter.log("Then Check the table personal details will show");
