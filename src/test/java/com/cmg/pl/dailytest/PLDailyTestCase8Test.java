...
public class PLDailyTestCase8Test {
	...
	public void testPLDailyTestCase8() throws Exception {
		...
		System.out.println("Case #8: Begin to load #8 member BPF-0001794(Security lock-out in place)!");
		try {
			...
			// Finish
			System.out.println("Case #8: Loading #8 member BPF-0001794(Security lock-out in place) PASS!");
			// ----------------------------------------
			// May take a snapshot here
			TakeScreenShot.takeScreenshoot();
		} catch (SeleniumException se) {
			retried++;
			System.out.println("retried case8:" + retried);
			System.out.println(se.getMessage());
			if (retried < 5) {
				testPLDailyTestCase8();
			} else {
				System.out.println("Case #8: Loading #8 member BPF-0001794(Security lock-out in place) FAILURE!");
			}
		}
