...
public class PLDailyTestCase4Test {
	...
	@Test
	public void testPLDailyTestCase4() throws Exception {
		// Begin to load #4 member BCF-0001532(Pensioner)!
		System.out.println("Case #4: Begin to load #4 member BCF-0001532(Pensioner)!");
		try {
			...
			selenium.waitForPageToLoad("30000");
			Thread.sleep(2000);
			selenium.select("id=bgroup", "label=BCF");
			selenium.type("id=crefno", "0001532");
			selenium.click("id=doauth");
			Thread.sleep(2000);
			...
			assertFalse(selenium.isTextPresent("[["));
			assertFalse(selenium.isTextPresent("]]"));
			assertTrue(selenium.isElementPresent("//span[@id='f_Refno_01'][contains(text(),'0001532')]"));
			...
			// Finish
			System.out.println("Case #4: Loading #4 member BCF-0001532(Pensioner) PASS!");
			// ----------------------------------------
			// May take a snapshot here
			TakeScreenShot.takeScreenshoot();
		} catch (SeleniumException se) {
			retried++;
			System.out.println("retried case4:" + retried);
			System.out.println(se.getMessage());
			if (retried < 5) {
				testPLDailyTestCase4();
			} else {
				// Finish
				System.out.println("Case #4: Loading #4 member BCF-0001532(Pensioner) FAILURE!");
				// ----------------------------------------
			}
		}
